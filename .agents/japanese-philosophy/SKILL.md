...
description: >-
  Apply authentic Japanese design philosophy — Ma, Kanso, Wabi-Sabi, Shibui, Fukinsei, Yūgen, Seijaku, Omotenashi — to any design surface: hero sections, landing pages, posters, brand identity, packaging, decks, UI micro-interactions. Use whenever the user wants a "Japanese", "MUJI-style", "zen", "quiet", "understated luxury", or "elegant minimalism that isn't flat/generic" aesthetic, or references Kenya Hara, MUJI, Ma, wabi-sabi, negative space as material, or "In Praise of Shadows". Also trigger with no word "Japanese" present, whenever a request describes negative space as active rather than leftover, emptiness that invites rather than just clears clutter, or a design that should feel expensive through restraint rather than ornament. This is the philosophy/decision layer — pair with this library's frontend-design, web-design-pro, svg, hero-section-architect, or creative-webgl skills for the actual code once direction is set.
---
...
Practically: if a design is empty and that emptiness has no job to do, it isn't Japanese minimalism — it's just unfinished. Every use of this skill should be able to answer "what is this space waiting for?"
...
## Step 0: Which Japan is this brief actually asking for?

Real Japanese design runs on a density paradox, and this skill only covers one side of it on purpose:
...
Default to the elite pole unless the brief is specifically a mass-market, trust-through-abundance e-commerce context aimed at Japanese users — in that narrow case, applying Ma-heavy whitespace would work against the goal, and the right move is to say so rather than force this skill's aesthetic onto it.

## Workflow

1. **Identify the medium** (web hero/full site, poster, brand identity, packaging, deck, UI micro-interaction). Read `references/applications.md` for how the principles execute differently per medium — don't assume web guidance transfers 1:1 to a poster.
2. **Select 2–4 principles that solve the actual problem**, not all nine at once. A cluttered hero needs Kanso + Ma. A flat, characterless brand needs Shibui + Wabi-Sabi. A static, dead-feeling layout needs Fukinsei. Piling on every principle at once produces the same incoherence this skill exists to prevent.
...
- Is there one clear hierarchy anchor, not two or three competing ones?
...
- `references/applications.md` — how the same principles translate differently across web, poster, brand identity, packaging, decks, and UI micro-interactions. Read for medium-specific execution.
