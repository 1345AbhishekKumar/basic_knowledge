...
**2. Surface-level style copy.** Adding a cherry-blossom motif, a thin serif, and some extra padding without the underlying subtractive discipline produces something that looks the part and reads as costume the moment anyone looks closely. This is worse than not attempting the aesthetic at all, because it signals the surface without the substance. If you can't articulate *why* a specific principle justifies a specific choice in the brief at hand, don't make the choice on the strength of the aesthetic alone.

**3. Blandness mistaken for Kanso.** Kanso is a process — remove until removing more would remove meaning — not an outcome defined by a low element count. Two colors and one typeface with no considered hierarchy is bland; two colors and one typeface where every remaining choice is load-bearing is Kanso. The test is whether removing one more thing would cost you something specific, not whether the count is already low.

**4. Wrong pole for the brief.** Applying elite-pole Ma-heavy restraint to a brief that's actually asking for trust-through-density (mass-market Japanese e-commerce, information-heavy comparison tools) works against the goal. Check Step 0 in `SKILL.md` before committing to this skill's direction at all.

**5. Digital wabi-sabi as literal sloppiness or glitch effects.** Wabi-sabi values intentional, bounded imperfection — visible craft, not visible carelessness. A subtle paper-grain overlay reads as intentional; misaligned elements or inconsistent spacing read as a bug regardless of the philosophical justification attached after the fact.
