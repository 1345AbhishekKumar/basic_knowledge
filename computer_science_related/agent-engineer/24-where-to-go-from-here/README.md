# Lesson 24: where to go from here

## Introduction

You made it. Twenty-four lessons covering everything from "what is an agent?" to building multi-agent systems, protocols, harnesses, loops, and governed state graphs with production infrastructure. That is a solid foundation.
...
| 16 | MCP Deep Dive | How MCP works under the hood, client-server transport, and security considerations. |
| 17 | Agent Skills | Packaging reusable domain expertise and workflows as portable skill modules. |
| 18 | Orchestrators | Managing agent control flow - deterministic workflow agents vs. dynamic LLM coordination. |
| 19 | Agent Harnesses | Decoupling model reasoning from runtime execution with sandboxing, interceptors, and eval harnesses. |
| 20 | Looping | The autonomous execution cycle - perceive, reason, act, observe - and state dynamics across turns. |
| 21 | Loop Engineering | Hardening loops against the 7 deadly failures with termination predicates, stagnation breakers, and compaction. |
| 22 | Graph Engineering | Governed computational topologies - nodes, edges, typed state schemas, reducers, and checkpoints. |
| 23 | Loop vs Graph | Architectural decision matrix, trade-offs between autonomy and governance, and hybrid production patterns. |

That is 23 lessons of rigorous engineering. If any of these summaries feel unfamiliar, go back and re-read that lesson before moving forward.
...
### Path 5: "i want to become a loop & graph systems architect"

You want to master stateful orchestration, runtime harnesses, and high-reliability hybrid agent architectures.

**Start here:**
1. Master the **Agent Harness** in Lesson 19. Understand how decoupling reasoning from execution makes systems auditable and resilient.
2. Study **Loop Engineering** (Lesson 21) to eliminate infinite runaway loops, stagnation, and context bloat.
3. Dive into **Graph Engineering** (Lesson 22) and build your first state machine using typed schemas, reducers, and checkpoint persistence.
4. Use the decision matrix in **Loop vs Graph** (Lesson 23) to design hybrid "Graph of Loops" architectures for complex enterprise requirements.

**Revisit:** Lessons 18 (Orchestrators), 19 (Agent Harnesses), 21 (Loop Engineering), 22 (Graph Engineering), 23 (Loop vs Graph)
...
[Previous Lesson: Loop vs Graph](../23-loop-vs-graph/) | [Back to Course Overview](../README.md)
